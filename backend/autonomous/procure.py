...
def _read_supplier(
    ...
    if reasoning is not None and buyer is not None:
        evidence_assessment = assess_supplier_evidence(
            source=source,
            reasoning=reasoning,
            buyer=buyer,
        )
...
def procure(
    ...
    if not ranked:
        ...
        stages["supplier_selection"] = "failed"

        code = _failure_code(
            stage="SUPPLIER_SELECTION",
            error="No supplier candidates were available.",
        )
    ...
    if not eligible_ranked:
        ...
        # This is an expected procurement outcome, not a server error:
        # every supplier was read and ranked, but none satisfied the
        # hard compatibility gate. Return a structured deadlock so the
        # UI can show exactly where READ -> REASON -> SELECT stopped and
        # why ACT/VERIFY were never attempted.
        best = all_ranked[0]
        failure_provenance = []
        best_flags = list(
            getattr(best, "risk_flags", None)
            or []
        )
        ...
        _record_failure(
            ...
            message=(
                "All supplier candidates fell below the minimum "
                f"compatibility score of {request.minimum_supplier_score:.2f}. "
                f"Best candidate {best.supplier.name} scored "
                f"{best.compatibility_score:.2f}. "
                + (
                    "Hard policy risks: "
                    + "; ".join(best_flags[:4])
                    if best_flags
                    else "No compatible supplier remained after policy validation."
                )
            ),
            recoverable=False,
        )
        ...
        stages["supplier_selection"] = "failed"
        stages["act"] = "blocked"
        stages["verify"] = "blocked"
        stages["recovery"] = "not_needed"
        ...
        return AutonomousProcurementResponse(
            ...
            anakin={
                ...
                "source_mode": (
                    "ANAKIN"
                    if anakin_sources_read
                    else "SIMULATION / NONE"
                ),
            },
            status="deadlock",
            verified=False,
            ...
            reasoning_history=[reasoning.model_dump()],
            recovery_events=[],
            failure_provenance=failure_provenance,
            supplier_ranking=[
                item.model_dump()
            ...
            selected_supplier=best.model_dump(),
            attempts=[],
            final_action=None,
            final_verification=None,
            failure_reason=(
                "Deadlock: no supplier satisfied the minimum "
                "compatibility threshold, so Nexora refused to "
                "enter negotiation."
            ),
            ...
            demo={
                ...
                "description": "Supplier-selection deadlock.",
            },
        )
